...
LABEL org.opencontainers.image.source="https://github.com/barrow1990/bazarr-mcp-server" \
...
# Liveness only (process up, HTTP serving) — not Bazarr connectivity, so a
# transient Bazarr outage doesn't get Dockhand/Docker restarting this
# container in a loop. Use GET /ready separately to check Bazarr connectivity.
HEALTHCHECK --interval=30s --timeout=5s --start-period=10s --retries=3 \
  CMD ["python", "-c", "\
import os, sys, urllib.request; \
port = os.environ.get('MCP_PORT', '8933'); \
